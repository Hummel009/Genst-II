...
public abstract class LOTRVillageGen {
	...
	private LocationInfo isVillageCentre(World world, int chunkX, int chunkZ) {
		...
		if (cacheLocation != null) {
			...
			i2 *= this.gridScale;
			k2 *= this.gridScale;
			i2 += MathHelper.getRandomIntegerInRange(villageRand, -this.gridRandomDisplace, this.gridRandomDisplace);
			k2 += MathHelper.getRandomIntegerInRange(villageRand, -this.gridRandomDisplace, this.gridRandomDisplace);
	...
	public void generateInChunk(World world, int i, int k) {
		...
		while (var5.hasNext()) {
			AbstractInstance<?> instance = (AbstractInstance) var5.next();
	...
	public void generateInstanceInChunk(AbstractInstance<?> instance, World world, int i, int k) {
		for (int i1 = i; i1 <= i + 15; ++i1) {
			for (int k1 = k; k1 <= k + 15; ++k1) {
				...
				while (var16.hasNext()) {
					StructureInfo struct = (StructureInfo) var16.next();
	...
	private int getTopTerrainBlock(World world, int i, int k, BiomeGenBase biome, boolean acceptSlab) {
		...
		while (true) {
			if (!world.getBlock(i, j + 1, k).getMaterial().isLiquid()) {
