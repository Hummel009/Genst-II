...
class LOTRVillageGenRhun(biome: LOTRBiome?, f: Float, flag: Boolean) : LOTRVillageGen(biome) {
	...
	class Instance(village: LOTRVillageGenRhun, world: World?, i: Int, k: Int, random: Random?, loc: LocationInfo?) :
		AbstractInstance<LOTRVillageGenRhun?>(village, world, i, k, random, loc) {
		...
		private fun setupTown(random: Random) {
			...
			addStructure(LOTRWorldGenEasterlingWell(false), wellX, wellZ, 3, true)
			addStructure(LOTRWorldGenEasterlingWell(false), wellZ, wellX, 0, true)
			addStructure(LOTRWorldGenEasterlingGatehouse(false).setSignText(villageName), 0, 94, 2, true)
			val towerX = 90
			addStructure(
				LOTRWorldGenEasterlingTower(false).disableDoor().setBackLadder().setLeftLadder(),
				-towerX,
		...
		private fun setupVillage(random: Random) {
			...
			val signX = 12
			addStructure(LOTRWorldGenEasterlingVillageSign(false).setSignText(villageName), -signX, 0, 1, true)
			addStructure(LOTRWorldGenEasterlingVillageSign(false).setSignText(villageName), signX, 0, 3, true)
		...
		override fun setupVillageProperties(random: Random) {
			...
			villageType =
				if (random.nextInt(4) == 0) VillageType.FORT else if (enableTowns && random.nextInt(4) == 0) VillageType.TOWN else VillageType.VILLAGE
		}
	}
}
