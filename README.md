...
> [!IMPORTANT]
> Мне лень писать отдельную документацию ради пары проектов, так что я напишу один раз тут: не используйте Eclipse IDE. Причина - конкретно в этом моде присутвует код на языке Kotlin, который в Eclipse IDE плохо поддерживается.
